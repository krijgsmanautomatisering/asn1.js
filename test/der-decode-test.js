...
describe('asn1.js DER decoder', function() {
  ...
  it('should decode implicit application', function() {
    const E = asn1.define('E', function() {
      this.implicit(7, 'application').seq().obj(this.key('id').int());
    });
    const decoded = E.decode(Buffer.from('6704020200ab', 'hex'));
    assert.equal(decoded.id.toString(10), 0xab);
  });
  it('should decode explicit application', function() {
    const E = asn1.define('E', function() {
      this.explicit(7, 'application').ia5str();
    });
    const decoded = E.decode(Buffer.from('6703160158', 'hex'));
    assert.equal(decoded, 'X');
  });
  it('should decode implicit explicit application', function() {
    const T = asn1.define('T', function() {
        this.explicit(22, 'application').ia5str();
    });
    const E = asn1.define('E', function() {
      this.seq().obj(
          this.key('val').implicit(7).use(T)
      );
    })
    const decoded = E.decode(Buffer.from('3003670158', 'hex'));
    console.log(decoded);
  });
});

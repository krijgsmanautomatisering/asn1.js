...
describe('asn1.js DER encoder', function() {
  ...
  it('should code explicit application tag as 0x67', function() {
    const E = asn1.define('E', function() {
      this.explicit(7, 'application').octstr();
    })
    const encoded = E.encode('X', 'der');

    // <Explicit tag> <wrapped len> <str tag> <len> <payload>
    assert.equal(encoded.toString('hex'), '6703040158');
    assert.equal(encoded.length, 5);
  })

  it('should code implicit application tag as 0x67', function() {
    const E = asn1.define('E', function() {
      this.implicit(7, 'application').seq().obj(this.key('id').int());
    })
    const encoded = E.encode({id:0xab}, 'der');

    // <Explicit tag> <wrapped len> <str tag> <len> <payload>
    assert.equal(encoded.toString('hex'), '6704020200ab');
    assert.equal(encoded.length, 6);
  })
  it('should force implicit choice tag to be explicit', function() {
    const A = asn1.define('A', function() {
      this.seq().obj(
          this.key('test').implicit(3).use(B)
      )
    });
    const B = asn1.define('B', function() {
      this.explicit(7, 'application').choice({
        item: this.octstr()
      });
    });
    let out = A.encode({
    ...
    assert.equal(out.toString('hex'), '3008a306040431323334');
    let obj = A.decode(out);
    assert.equal(obj.test.value.toString(), '1234');
